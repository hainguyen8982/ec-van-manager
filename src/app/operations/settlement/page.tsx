        ...
        <button
          ...
          onClick={async () => {
            ...
            if (newSettlement) {
              ...
              const msg = `🧾 <b>[CHỐT SỔ THÀNH CÔNG]</b>\nKỳ: ${new Date(stats.startDate).toLocaleDateString('vi-VN')} - ${new Date(stats.endDate).toLocaleDateString('vi-VN')} (${stats.daysCount} ngày)\n\nLợi nhuận ròng: <b>${netProfit >= 0 ? '+' : ''}${fmt(netProfit)} đ</b>\n${reserveAllocation > 0 ? `Trích Quỹ dự phòng: <b>-${fmt(reserveAllocation)} đ</b>\n` : ''}💰 <b>TỔNG CHUYỂN GÓC ĐẦU TƯ: ${fmt(ownerAmount + reserveAllocation)} đ</b>\n<i>(Bao gồm Lãi ${fmt(ownerAmount)} đ + Quỹ dự phòng ${fmt(reserveAllocation)} đ)</i>\n\nHãy vào phần Quản lý để xem Báo cáo chi tiết.`;
